...
public class DCMPL extends gov.nasa.jpf.jvm.bytecode.DCMPL {
  ...
  public Instruction execute(ThreadInfo ti) {
    ...
    Pair<Double> right = ConcolicUtil.popDouble(sf);
    Pair<Double> left = ConcolicUtil.popDouble(sf);
    ...
    Pair<Integer> result = new Pair<Integer>(conditionValue, new NumericCMP(
            left.symb, right.symb));
    
    ConcolicUtil.pushInt(result, sf);
    
    if (ConcolicInstructionFactory.DEBUG) ConcolicInstructionFactory.logger.finest("Execute DCMPL (" + left.conc + " CMP " + right.conc + "): " + conditionValue + " [[" + result + "]]");		
    return getNext(ti);
  }
}
